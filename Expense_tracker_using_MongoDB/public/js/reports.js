...
async function showMonthlyReport() {
    ...
    const token  = localStorage.getItem('token');
    const { data } = await axios.post("/report/monthreport", monthly, { headers: {"Authorization" : token} });
...
async function handleDownload() {
    const token = localStorage.getItem('token');
    try {
        const response = await axios.get('/report/download', {
...
async function showPrevReport(){
    const token = localStorage.getItem('token');
    try {
     const {data} = await axios.get('/report/prevReport',{
        headers: { "Authorization": token }
     })
